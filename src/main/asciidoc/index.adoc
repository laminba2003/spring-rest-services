= Service Documentation
Mamadou Lamine Ba <https://github.com/laminba2003[@laminba2003]>
:revnumber: {project-version}
:example-caption!:
ifndef::imagesdir[:imagesdir: images]
ifndef::sourcedir[:sourcedir: ../../main/java]
:description: This developer guide describes how to use this service.
:keywords: Java, Spring, reference, learn, how to
:doctype: book
:page-layout!:
:toc: left
:nofooter:
:sectanchors:
:sectlinks:
:sectnums:
:icons: font
:source-highlighter: highlightjs
:source-language: asciidoc
:docinfo: shared-head


== Country Controller

[plantuml, GetCountries-class-diagram, png]
----
class Country {

}

class CountryEntity {

}

interface CountryRepository<CountryEntity, Long> <<Repository>> {
}

interface CrudRepository<T,ID> {
}

class CountryService <<Service>> {
}

class CountryController <<Controller>> {
}


CountryEntity ..> Country
CountryRepository ..> CountryEntity
CrudRepository <|-- CountryRepository
CountryService --> CountryRepository
CountryController --> CountryService

----

=== GetCountries endpoint

[plantuml, GetCountries-sequence-diagram, png]
----

participant client
participant countryController
participant countryService
participant countryRepository
participant database

client -> countryController: GET /countries
countryController -> countryService: invoke the getCountries method of the service
countryService -> countryRepository: invoke the findAll method of the repository
countryRepository -> database: hits the database to get the list of countries
countryRepository <-- database: return
countryService <-- countryRepository: return
countryService -> countryService: converts the Entities to DTOs
countryController <-- countryService: return
countryController -> client: return response as JSON

----


.request
include::{snippets}/getCountries/curl-request.adoc[]

.response
include::{snippets}/getCountries/http-response.adoc[]


=== GetCountry endpoint

.request
include::{snippets}/getCountry/curl-request.adoc[]

.response
include::{snippets}/getCountry/http-response.adoc[]


== Person Controller

[plantuml, GetPersons-class-diagram, png]
----
class Person {
  + void setId(Long id)
  + Long getId()
  + void setFirstName(String firstName)
  + String getFirstName()
}

class PersonEntity {

}

interface PersonRepository<PersonEntity, Long> <<Repository>> {
}

interface CrudRepository<T, ID> {
}

class PersonService <<Service>> {
}

class PersonController <<Controller>> {
}


PersonEntity ..> Person
PersonRepository --> PersonEntity
CrudRepository <|-- PersonRepository
PersonService --> PersonRepository
PersonController --> PersonService

----

=== GetPersons endpoint


.request
include::{snippets}/getPersons/curl-request.adoc[]

.response
include::{snippets}/getPersons/http-response.adoc[]